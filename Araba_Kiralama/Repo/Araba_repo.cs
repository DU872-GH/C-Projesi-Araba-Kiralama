...
namespace araba_kiralama.Repo
{
   ...
   @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\deniz\Desktop\C-Projesi-Araba-Kiralama-main\Araba_Kiralama\Araba_Kiralama\Veritabani\Araclar.mdf;Integrated Security=True;";
        ...
        public int GetGunlukUcret(string plakaNo)
        {
            try
            {
                using SqlConnection connection = new SqlConnection(ConnectionString);
                ...
                string sql = "SELECT gunlukUcret FROM arabalar WHERE plaka = @plaka";

                using SqlCommand command = new SqlCommand(sql, connection);
                command.Parameters.AddWithValue("@plaka", plakaNo);

                using SqlDataReader reader = command.ExecuteReader();
                if (reader.Read())
                {
                    return reader.GetInt32("gunlukUcret");
                }
            }
            catch (Exception ex)
            {
        ...
        public void SetKirada(string plaka)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        command.Parameters.AddWithValue("@kirada", 0);
        ...
        public void SetKirada2(string plaka)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        command.Parameters.AddWithValue("@kirada", 1);
                        command.Parameters.AddWithValue("@plaka", plaka);
                        command.ExecuteNonQuery();
                    }
                }
